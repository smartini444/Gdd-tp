...
/*CREATE FUNCTION obtenerMedioDePago(@nroUsuario int) RETURNS nvarchar(50)
AS	
	BEGIN
		DECLARE @pedido_nro int;
		DECLARE @medio_pago_nro int;
		DECLARE @medio_pago nvarchar(50);
		SELECT @pedido_nro = PEDIDO_USUARIO_NRO FROM NEW_MODEL.PEDIDO WHERE PEDIDO_USUARIO_NRO = @nroUsuario
		SELECT @medio_pago_nro = PAGO_MEDIO_PAGO_NRO FROM NEW_MODEL.PAGO WHERE PAGO_PEDIDO_NRO = @pedido_nro
		SELECT @medio_pago = MEDIO_PAGO FROM NEW_MODEL.MEDIO_PAGO WHERE MEDIO_PAGO_NRO = @medio_pago_nro
		return @medio_pago;
	END
GO
