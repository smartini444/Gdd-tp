...
IF EXISTS(SELECT [name] FROM sys.objects WHERE [name] = 'dropear_tablas')
...
GO
CREATE PROCEDURE MIGRAR_LOCALIDADES2
AS
    BEGIN
        DECLARE @count INT;
...
CREATE PROCEDURE MIGRAR_REPARTIDOR
	...
	BEGIN
		...
		SELECT @usuario_nro = USUARIO_DNI FROM NEW_MODEL.USUARIO WHERE USUARIO_DNI  = @usuarioDni;
		RETURN @usuario_nro;
	END
GO  
