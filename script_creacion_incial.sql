...
GO
CREATE FUNCTION obtenerLocalidadNro(@localidadProvinciaNombre nvarchar(255), @localidadNombre nvarchar(255)) RETURNS int 
AS
    BEGIN
        DECLARE @LocalidadId int;
        SELECT @LocalidadId = LOCALIDAD_NRO FROM NEW_MODEL.LOCALIDAD JOIN NEW_MODEL.PROVINCIA ON LOCALIDAD_PRIVINCIA_NRO = PROVINCIA_NRO WHERE LOCALIDAD_NOMBRE = @LocalidadNombre AND PROVINCIA_NOMBRE = @localidadProvinciaNombre;
        RETURN @LocalidadId;
    END
GO
...
IF OBJECT_ID('obtenerLocalidadNROO', 'FN') IS NOT NULL
DROP FUNCTION  obtenerLocalidadNROO;
GO
CREATE FUNCTION  obtenerLocalidadNROO(@provincia nvarchar(255),@localidad nvarchar(255)) RETURNS int
AS
BEGIN
...
--         SELECT dbo.obtenerUsuarioNro(USUARIO_DNI), dbo.obtenerLocalidadNro(DIRECCION_USUARIO_LOCALIDAD,DIRECCION_USUARIO_PROVINCIA), DIRECCION_USUARIO_NOMBRE, DIRECCION_USUARIO_DIRECCION FROM gd_esquema.Maestra WHERE DIRECCION_USUARIO_DIRECCION IS NOT NULL GROUP BY DIRECCION_USUARIO_DIRECCION, DIRECCION_USUARIO_NOMBRE, USUARIO_DNI
--     END
-- GO
...
IF EXISTS(SELECT [name] FROM sys.procedures WHERE [name] = 'MIGRAR_LOCALIDADES2')
DROP PROCEDURE MIGRAR_LOCALIDADES2
...
IF EXISTS(SELECT [name] FROM sys.all_objects WHERE [name] = 'obtenerLocalidadNROO')
DROP FUNCTION obtenerLocalidadNROO
END
GO