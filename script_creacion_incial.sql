...
GO
CREATE FUNCTION obtenerMensajeriaNro(@dirDest nvarchar(50),@fecha datetime,@km decimal(18,2),@fechaEntrega datetime) RETURNS int
AS
	BEGIN
		DECLARE @numeroEnvio int;
		SELECT @numeroEnvio = ENVIO_MENSAJERIA_NRO FROM NEW_MODEL.ENVIO_MENSAJERIA WHERE ENVIO_MENSAJERIA_DIR_DEST = @dirDest AND ENVIO_MENSAJERIA_FECHA = @fecha AND  ENVIO_MENSAJERIA_KM = @km AND ENVIO_MENSAJERIA_FECHA_ENTREGA = @fechaEntrega;
		RETURN @numeroEnvio;
	END
GO
