...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_PEDIDO_ENVIO_REPARTIDOR_FECHA')
    DROP INDEX IX_PEDIDO_ENVIO_REPARTIDOR_FECHA ON NEW_MODEL.PEDIDO_ENVIO;
    CREATE INDEX IX_PEDIDO_ENVIO_REPARTIDOR_FECHA
    ON NEW_MODEL.PEDIDO_ENVIO (PEDIDO_ENVIO_REPARTIDOR_NRO, PEDIDO_ENVIO_FECHA);

    IF EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_ENVIO_MENSAJERIA_REPARTIDOR_FECHA')
    DROP INDEX IX_ENVIO_MENSAJERIA_REPARTIDOR_FECHA ON NEW_MODEL.ENVIO_MENSAJERIA;
    CREATE INDEX IX_ENVIO_MENSAJERIA_REPARTIDOR_FECHA
    ON NEW_MODEL.ENVIO_MENSAJERIA (ENVIO_MENSAJERIA_REPARTIDOR_NRO, ENVIO_MENSAJERIA_FECHA);
...
AS  
    ...
    SELECT @pedidoEnvioNro = PEDIDO_ENVIO_NRO FROM NEW_MODEL.PEDIDO_ENVIO WHERE PEDIDO_ENVIO_REPARTIDOR_NRO = @repartidorNro AND PEDIDO_ENVIO_FECHA = @fecha;
...
BEGIN
	...
	SELECT DISTINCT dbo.obtenerUsuarioNro(USUARIO_DNI),dbo.obtenerRepartidorNro(REPARTIDOR_DNI),dbo.obtenerMedioDePago(MEDIO_PAGO_TIPO),dbo.obtenerPaqueteNro(PAQUETE_TIPO),dbo.obtenerMensajeriaNro(ENVIO_MENSAJERIA_DIR_DEST,PEDIDO_TIEMPO_ESTIMADO_ENTREGA,ENVIO_MENSAJERIA_PRECIO_ENVIO),dbo.obtenerEstadoNro(ENVIO_MENSAJERIA_ESTADO),ENVIO_MENSAJERIA_TOTAL,ENVIO_MENSAJERIA_OBSERV,ENVIO_MENSAJERIA_FECHA ,ENVIO_MENSAJERIA_FECHA_ENTREGA ,ENVIO_MENSAJERIA_VALOR_ASEGURADO ,ENVIO_MENSAJERIA_CALIFICACION,ENVIO_MENSAJERIA_PRECIO_SEGURO
 FROM gd_esquema.Maestra
	WHERE ENVIO_MENSAJERIA_NRO IS NOT NULL
END
GO
...
IF EXISTS(SELECT [name] FROM sys.all_objects WHERE [name] = 'obtenerMensajeriaNro')
